...
type Options struct {
	...
	Debug bool
	// LogPackage is an external log packages that's enable several log levels
	LogPackage Logger
}
...
func (internalLog) Logf(str string, atr ...interface{}) {
	fmt.Printf(str, atr)
}
...
func New(options Options) *Cors {
	...
	if options.Debug {
		if options.LogPackage != nil {
			c.Log = options.LogPackage
		} else {
			c.Log = internalLog{}
		}
	}
